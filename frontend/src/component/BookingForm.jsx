...
const BookingForm = ({ selectedPackage, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-gradient-to-b from-stone-800 to-stone-900 rounded-3xl shadow-2xl w-full max-w-2xl relative overflow-y-auto max-h-[90vh] border border-orange-500/30">
        
        {/* Close Button */}
        <button
          onClick={onClose}
          disabled={isSubmitting}
          className="absolute top-4 right-4 z-10 p-2 bg-black/40 backdrop-blur-sm rounded-full text-gray-300 hover:text-white hover:bg-orange-600/60 transition-all duration-300 border border-white/10"
        >
          <FaTimes className="w-5 h-5" />
        </button>

        {/* Form Header */}
        <div className="relative h-32 bg-gradient-to-r from-orange-600 to-amber-600 p-6">
          <h2 className="text-2xl font-bold text-white mb-1">Book Your Adventure</h2>
          <p className="text-orange-100">{selectedPackage.title}</p>
          <div className="absolute -bottom-6 left-6 w-12 h-12 bg-stone-800 rounded-full border-4 border-orange-500 flex items-center justify-center">
            <FaAward className="w-6 h-6 text-orange-400" />
          </div>
        </div>
          ...
          <div className="bg-white/5 rounded-xl p-4 mb-6 border border-white/10">
            <div className="flex justify-between items-center">
              ...
              <div className="text-right">
                ...
                <p className="text-xl font-bold text-orange-400">N${selectedPackage.newPrice}</p>
                <p className="text-xs text-gray-400">per person</p>
              </div>
            </div>
            {selectedPackage.note && (
              <p className="text-xs text-orange-300 mt-2 italic border-t border-white/10 pt-2">
                {selectedPackage.note}
              </p>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                required
                disabled={isSubmitting}
                className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition disabled:opacity-50"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-1">
                  <FaCalendar className="inline mr-2 text-orange-400" />
                  Preferred Date *
                </label>
                <input
                  type="date"
                  ...
                  required
                  disabled={isSubmitting}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 transition disabled:opacity-50"
                />
              </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">
                <FaComment className="inline mr-2 text-orange-400" />
                Special Requests
              </label>
              <textarea
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="flex-1 px-6 py-3 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white font-semibold rounded-xl transition-all duration-300 transform hover:scale-105 shadow-lg disabled:opacity-50 disabled:hover:scale-100"
              >
